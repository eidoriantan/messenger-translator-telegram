...
describe('User Database', () => {
  ...
  it('Adds user', async () => {
    const userData = await users.addUser({
      id: userId,
      username: 'Test Name',
      language_code: 'en'
    })

    userData.should.containEql({ id: userId })
    userData.should.containEql({ name: 'Test Name' })
    userData.should.containEql({ language: 'en' })
    userData.should.containEql({ locale: 'en' })
    userData.should.containEql({ message: 0 })
    testUser = userData
  })

  it('Gets user', async () => {
    const userData = await users.getUser(testUser.id)
  ...
  after(async () => {
    await users.deleteUser(testUser.id)
    await feedbacks.deleteFeedback(testUser.id)
    database.close()
  })
})
